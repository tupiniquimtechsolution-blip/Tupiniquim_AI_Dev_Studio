...
if ([System.IO.Path]::GetPathRoot($ProjectRoot).TrimEnd('\') -ne 'E:') {
  throw "Este projeto deve ser executado no disco E:. Caminho atual: $ProjectRoot"
}
...
$Corepack = Join-Path $Bin 'corepack.cmd'
& $Corepack pnpm install
if ($LASTEXITCODE -ne 0) { throw "pnpm install falhou com código $LASTEXITCODE" }

Write-Host 'Bootstrap concluído. Stores, caches e temporários estão no disco E.'
