...
if (-not (Test-Path -LiteralPath $Node -PathType Leaf)) { throw "Node.js do SSD não encontrado em $Node" }
if (-not (Test-Path -LiteralPath $Corepack -PathType Leaf)) { throw "Corepack do SSD não encontrado em $Corepack" }
if (-not (Test-Path -LiteralPath $Pnpm -PathType Leaf)) { throw "pnpm do SSD não encontrado em $Pnpm" }
if (-not (Test-Path -LiteralPath (Join-Path $DotnetRoot 'dotnet.exe') -PathType Leaf)) { throw "SDK .NET/Visual Basic do SSD não encontrado em $DotnetRoot" }
...
Write-Host 'Bootstrap concluído. Projeto, runtimes, caches, dados e temporários estão no SSD F:.'
