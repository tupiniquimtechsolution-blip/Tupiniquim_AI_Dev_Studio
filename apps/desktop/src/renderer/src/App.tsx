...
export const App = (): React.JSX.Element => {
  ...
  const sendToAgent = async (): Promise<void> => {
    ...
    if (mode === 'PLAN') {
      ...
      if (planResult.ok) {
        ...
        setConversation((current) => [...current, { id: crypto.randomUUID(), role: 'assistant', text: 'Plano persistido. Revise os passos e aprove somente os efeitos desejados.', turnId: null, complete: true }])
  ...
  const savePlan = async (): Promise<void> => {
    ...
    const result = await window.studio.planning.update({ plan: planned.plan })
      ...
      <div className="workbench">
        ...
        <aside className="agent-panel panel">
          ...
          <section className="agent-conversation">
            <div className="agent-message"><span className="message-label">SISTEMA</span><p>Codex usa stdio JSONL, dados em F:\CODEX e execução read-only nesta onda. Mutações aguardam aprovação granular.</p></div>
