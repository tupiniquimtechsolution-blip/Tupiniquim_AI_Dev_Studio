...
export const detectPrivateEnvironmentPresence = async (projectRoot: string, names: string[]): Promise<Record<string, boolean>> => {
  const requested = new Set(names)
  const presence = Object.fromEntries(names.map((name) => [name, false]))
  let source: string
  try { source = await readFile(path.join(projectRoot, '.env.local'), 'utf8') } catch { return presence }
  for (const rawLine of source.split(/\r?\n/u)) {
    const separator = rawLine.indexOf('=')
    if (separator < 1) continue
    const name = rawLine.slice(0, separator).trim()
    if (!requested.has(name)) continue
    const valuePresent = rawLine.slice(separator + 1).trim().replace(/^['"]|['"]$/gu, '') !== ''
